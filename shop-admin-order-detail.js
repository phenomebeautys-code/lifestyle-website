/* shop-admin-order-detail.js */
import { state, BADGE_MAP, STATUS_LABELS, SVG, esc, showToast } from './shop-admin-core.js';
import { getDeliveryLabel } from './shop-admin-dashboard.js';
import { updateOrderStatus } from './shop-admin-orders.js';
...
export function openOrderDetail(orderId) {
  ...
  document.getElementById('orderDetailBody').innerHTML = `
    ...
    <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:4px">
      ...
      <button class="btn btn-secondary" style="flex:1;min-width:120px;justify-content:center" onclick="printOrderLabelById('${o.id}')">Print Label</button>
    </div>
...
export async function updateFromDetail(orderId) {
...
/* ─── PRINT LABEL ─────────────────────────────── */
export function printOrderLabelById(orderId) {
  const order = state.allOrders.find(x => x.id === orderId);
  if (order) printLabel(order);
}
...
export function closePrintLabel() {
